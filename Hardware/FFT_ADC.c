...
//	unsigned char i=0,flag=0;
	...
	FFT_Start();
	FFT_Pro();
	display((unsigned int *)lBufMagArray);
//	FFT_Stop();
//	display2();
...
//		FFT_Start();
//	}
//	OLED_Printf(2,0,"%d %d %d",lBufMagArray[1],adc_dma_flag,adc_buf[0]);
...
void GetPowerMag(void)
{
    ...
    for(i=0; i<NPT/2; i++)
    {
        lX  = (lBufOutArray[i] << 16) >> 18;
        lY  = (lBufOutArray[i] >> 16);
			
				//除以32768再乘65536是为了符合浮点数计算规律
        X = NPT * ((float)lX) / 32768;
        Y = NPT * ((float)lY) / 32768;
        Mag = (sqrt(X * X + Y * Y)*2.0)/ NPT;
        if(i == 0)	
...
//	for(i=0;i<NPT;i++)
//	{
//		OLED_Printf(2,2,"%d",lBufMagArray[i]);
//	}
}
void display(unsigned int *p1)
{
	...
	for(i=0;i<128;i++)
	{
		...
		OLED_SPI_Fill(i,0,i,1+(*p2/16),1);
		OLED_SPI_Fill(i,1+(*p2/16),i,63,0);
//		OLED_Refresh_Gram();
		p2++;
	}
//	OLED_Clear();
	
	OLED_SPI_Refresh_Gram();
//	Delay_ms(500);
}
